...
@interface XInternalRequestState : NSObject
...
@implementation XInternalRequestState

@end


@interface XNetworkObserver (NSURLConnectionHelpers)
...
@interface XNetworkObserver (NSURLSessionTaskHelpers)
...
@interface XNetworkObserver ()

@property (nonatomic, strong) NSMutableDictionary<NSString *, XInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic, strong) dispatch_queue_t queue;

@end

@implementation XNetworkObserver
...
+ (BOOL)isEnabled
{
    return [[[NSUserDefaults standardUserDefaults] objectForKey:kXNetworkObserverEnabledDefaultsKey] boolValue];
}
...
{
    static XNetworkObserver *sharedObserver = nil;
...
{
    return NSSelectorFromString([NSString stringWithFormat:@"_X_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)]);
}
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector sniffingBlock:(void (^)(void))sniffingBlock originalImplementationBlock:(void (^)(void))originalImplementationBlock
{
    ...
    // See https://github.com/Flipboard/X/issues/61 for an example.
    if (!object) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [XNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [XNetworkObserver swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);
        
        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [[XNetworkObserver sharedObserver] connectionWillCancel:slf];
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^asyncSwizzleBlock)(Class, NSURLRequest *, NSOperationQueue *, NSURLConnectionAsyncCompletion) = ^(Class slf, NSURLRequest *request, NSOperationQueue *queue, NSURLConnectionAsyncCompletion completion) {
            if ([XNetworkObserver isEnabled]) {
                NSString *requestID = [self nextRequestID];
                //将要开始请求
                [[XHttpRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:nil];
                NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
                [[XHttpRecorder defaultRecorder] recordMechanism:mechanism forRequestID:requestID];
                NSURLConnectionAsyncCompletion completionWrapper = ^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                    //记录response和data
                    [[XHttpRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:response];
                    [[XHttpRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                    if (connectionError) {
                        //请求失败
                        [[XHttpRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:connectionError];
                    } else {
                        //请求结束
                        [[XHttpRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                    }
        ...
        [XNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSData *(^syncSwizzleBlock)(Class, NSURLRequest *, NSURLResponse **, NSError **) = ^NSData *(Class slf, NSURLRequest *request, NSURLResponse **response, NSError **error) {
            ...
            if ([XNetworkObserver isEnabled]) {
                ...
                [[XHttpRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:temporaryResponse];
                [[XHttpRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
                if (temporaryError) {
                    [[XHttpRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:temporaryError];
                } else {
                    [[XHttpRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                }
                if (error) {
        ...
        [XNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:syncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([XNetworkObserver instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionTask *(^asyncDataOrDownloadSwizzleBlock)(Class, id, NSURLSessionAsyncCompletion) = ^NSURLSessionTask *(Class slf, id argument, NSURLSessionAsyncCompletion completion) {
                ...
                if ([XNetworkObserver isEnabled] && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
            ...
            [XNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncDataOrDownloadSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (void)injectIntoNSURLSessionAsyncUploadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            [XNetworkObserver replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncUploadTaskSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (NSURLSessionAsyncCompletion)asyncCompletionWrapperForRequestID:(NSString *)requestID mechanism:(NSString *)mechanism completion:(NSURLSessionAsyncCompletion)completion
{
    NSURLSessionAsyncCompletion completionWrapper = ^(id fileURLOrData, NSURLResponse *response, NSError *error) {
        ...
        [[XHttpRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:[data length]];
        if (error) {
            [[XHttpRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:error];
        } else {
            [[XHttpRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
        }
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionWillSendRequestBlock undefinedBlock = ^NSURLRequest *(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLRequest *request, NSURLResponse *response) {
        [[XNetworkObserver sharedObserver] connection:connection willSendRequest:request redirectResponse:response delegate:slf];
        return request;
    };
    ...
    [XNetworkObserver replaceImplementationOfSelector:selector withSelector:swizzledSelector forClass:cls withMethodDescription:methodDescription implementationBlock:implementationBlock undefinedBlock:undefinedBlock];
}
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLResponse *response) {
        [[XNetworkObserver sharedObserver] connection:connection didReceiveResponse:response delegate:slf];
    };
...
+ (void)injectDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveDataBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSData *data) {
        [[XNetworkObserver sharedObserver] connection:connection didReceiveData:data delegate:slf];
    };
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFinishLoadingBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [[XNetworkObserver sharedObserver] connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectDidFailWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFailWithErrorBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSError *error) {
        [[XNetworkObserver sharedObserver] connection:connection didFailWithError:error delegate:slf];
    };
...
+ (void)injectTaskWillPerformHTTPRedirectionIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionWillPerformHTTPRedirectionBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSHTTPURLResponse *response, NSURLRequest *newRequest, void(^completionHandler)(NSURLRequest *)) {
        [[XNetworkObserver sharedObserver] URLSession:session task:task willPerformHTTPRedirection:response newRequest:newRequest completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveDataBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSData *data) {
        [[XNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveData:data delegate:slf];
    };
...
+ (void)injectDataTaskDidBecomeDownloadTaskIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidBecomeDownloadTaskBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLSessionDownloadTask *downloadTask) {
        [[XNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didBecomeDownloadTask:downloadTask delegate:slf];
    };
...
+ (void)injectTaskDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLSessionDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLResponse *response, void(^completionHandler)(NSURLSessionResponseDisposition disposition)) {
        [[XNetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveResponse:response completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidCompleteWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionTaskDidCompleteWithErrorBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSError *error) {
        [[XNetworkObserver sharedObserver] URLSession:session task:task didCompleteWithError:error delegate:slf];
    };
...
+ (void)injectDownloadTaskDidFinishDownloadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidFinishDownloadingBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, NSURL *location) {
        ...
        [[XNetworkObserver sharedObserver] URLSession:session task:task didFinishDownloadingToURL:location data:data delegate:slf];
    };
...
+ (void)injectDownloadTaskDidWriteDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidWriteDataBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
        [[XNetworkObserver sharedObserver] URLSession:session downloadTask:task didWriteData:bytesWritten totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite delegate:slf];
    };
...
static char const * const kXRequestIDKey = "kXRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask
{
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kXRequestIDKey);
    if (!requestID) {
...
{
    objc_setAssociatedObject(connectionOrTask, kXRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id)init
{
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.X.XNetworkObserver", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (XInternalRequestState *)requestStateForRequestID:(NSString *)requestID
{
    XInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [[XInternalRequestState alloc] init];
...
@implementation XNetworkObserver (NSURLConnectionHelpers)

- (void)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)response delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        XInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        requestState.request = request;
        [[XHttpRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:response];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        XInternalRequestState *requestState = [self requestStateForRequestID:requestID];
        [[XHttpRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:requestState.dataAccumulator];
...
@implementation XNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        [[XHttpRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        if (!requestState.dataAccumulator) {
            ...
            [[XHttpRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:downloadTask.response];
            
            NSString *requestMechanism = [NSString stringWithFormat:@"NSURLSessionDownloadTask (delegate: %@)", [delegate class]];
            [[XHttpRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
        }
        
        [[XHttpRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:bytesWritten];
    }];
}
...
- (void)URLSessionTaskWillResume:(NSURLSessionTask *)task
{
    ...
    [self performBlock:^{
        ...
        if (!requestState.request) {
            ...
            [[XHttpRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:task.currentRequest redirectResponse:nil];
        }
    }];
}
@end
